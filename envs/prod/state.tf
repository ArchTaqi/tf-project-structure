...
resource "aws_kms_alias" "key-alias" {
  name          = "alias/name-prod-terraform-key"
  target_key_id = aws_kms_key.terraform-bucket-key.key_id
}

resource "aws_s3_bucket" "terraform-state" {
  bucket = "name-prod-terraform"
}
...
terraform {
  backend "s3" {
    ...
    region         = "eu-north-1"
    encrypt        = true
    kms_key_id     = "alias/name-prod-terraform-key"
    dynamodb_table = "name-prod-terraform"
    profile        = "name-prod-terraform"
  }
}
