...
resource "aws_kms_alias" "key-alias" {
  name          = "alias/name-stage-terraform-key"
...
terraform {
  backend "s3" {
    ...
    region         = "eu-north-1"
    encrypt        = true
    kms_key_id     = "alias/name-stage-terraform-key"
    dynamodb_table = "name-stage-terraform"
    profile        = "name-stage-terraform"
  }
}
